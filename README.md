# NEUL v0.37 — Auto Sources + Complete 3D + UI Audit
...
- NTSU Arena / 林口體育館 now keeps FLOOR + LOWER + MIDDLE + UPPER structural layers. Event-specific seating maps can add or overlay zones but can no longer erase the physical lower bowl or activity floor.
- Kaohsiung Arena similarly retains a 1F configurable activity-floor layer in addition to fixed bowl tiers.
- The same base-venue invariant is applied globally: an event layout overlays the venue model; it never replaces the full venue structure.
...
## v0.39 automation
活動資料管線改為 event → seat map → section prices → 3D；API 會回傳每場 automation 狀態。Featured 每 10 秒自動輪播，並跟隨最新活動資料。資料列顯示上次同步與下次預計同步時間。
