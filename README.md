# NEUL v0.40.14 — Verified 316 + Vercel CPU Saver

本版將公開行事曆的 316 場視為候選 coverage 目標，不直接當成官方真相；先採主行事曆 fast-path，覆蓋不足才逐月補掃。Vercel 單次抓取偏少時，可合併上一份 healthy upcoming snapshot，但快取只保存未結束活動，不累積舊場。演藝新聞改為近 7 天、最新到最舊，先顯示 8 則並可逐批「看更多」，最多 24 則近期資料。Archive 仍只保留最近 20 場。
