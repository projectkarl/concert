...
function updateFeaturedAutoplayProgress(){
  const bar=$("#featuredProgressBar"), status=$("#featuredAutoStatus");
  const items=featuredEvents();
  if(!bar||!status) return;
  if(items.length<=1){ bar.style.width="0%"; status.textContent="AUTO"; return; }
  ...
  status.textContent=`AUTO ${Math.max(1,Math.ceil(remaining/1000))}s`;
}
...
function officialSeatMapSourceForCurrentLayout() {
  ...
  const sourcePage=event.sourceUrl || event.ticketUrl || event.ticketSourceUrl || layout.sourceUrl || machineRaw || "";
