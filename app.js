...
import { analyzeSeatMap } from "./seat-map-intelligence.js";
...
async function hydrateSeatMapGeometry(events=[]) {
  ...
  const analyzeOne=async event=>{
    try{
      ...
      const analysis=await analyzeSeatMap(event,model); if(!analysis?.hash) return false;
  ...
  // Analyze every current event that has an official map, but keep browser work bounded to three images
  // at a time so Safari/mobile remains responsive. CDN/proxy caching keeps the six-hour refresh light.
  for(let i=0;i<candidates.length;i+=3){
    const batch=await Promise.all(candidates.slice(i,i+3).map(analyzeOne));
