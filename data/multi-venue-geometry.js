...
export function sectionTicketLabel(layoutId, sectionId) {
  ...
  if (!section?.group || !layout.priceLabels) return null;
  return layout.priceLabels[section.group] || null;
}
export function venueSectionWarning(venueId, sectionId, row, layoutId, viewer={}) {
  ...
  if (layoutId===getVenueModel(venueId).baseLayoutId || !layout.eventId) messages.push('目前為場館區域級模擬；本場專屬舞台圖公布後會再校正。');
  return {level,messages};
}
