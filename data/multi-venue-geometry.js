...
export function ensureAutoEventLayout(event={}) {
  ...
  if (current?.eventId && !current.autoGenerated) {
    // Preserve hand-calibrated geometry/distances, but keep structured event metadata in sync.
    if (Array.isArray(event.sectionPriceRules) && event.sectionPriceRules.length) current.sectionPriceRules=JSON.parse(JSON.stringify(event.sectionPriceRules));
    if (event.price) current.priceSummary=event.price;
    if (event.seatLayoutSourceUrl) current.latestSeatLayoutSourceUrl=event.seatLayoutSourceUrl;
