...
export function ensureAutoEventLayout(event={}) {
  ...
  if (!venueLayouts[id]) {
    ...
    layout.seatMapAutoRegenerate=true;
...
export function applyAutoSeatMapAnalysis(layoutId, analysis={}) {
  ...
  layout.extraStageRects=Array.isArray(analysis.extraStageRects)?JSON.parse(JSON.stringify(analysis.extraStageRects)):[];
  if(Array.isArray(analysis.sections) && analysis.sections.length>=1){
    layout.sections=JSON.parse(JSON.stringify(analysis.sections));
    layout.tiers=JSON.parse(JSON.stringify(analysis.tiers||[]));
    const priceLabels={};
    for(const sec of analysis.sections) if(sec.autoPrice) priceLabels[sec.group]=sec.autoPrice;
    layout.priceLabels={...(layout.priceLabels||{}),...priceLabels};
    // Only the flexible/event floor is replaced by the official-map floor blocks.
    // Every fixed lower/middle/upper bowl remains visible so a new event can never
    // accidentally erase the physical venue.
    layout.replaceStructuralTiers=[...new Set([...(layout.replaceStructuralTiers||[]),'FLOOR'])];
  }else if(layout.autoGenerated){
    // If a new map contains stage geometry but no reliable floor blocks, keep the full
    // venue base rather than hiding structural seating.
    layout.sections=undefined;
    layout.tiers=undefined;
    layout.replaceStructuralTiers=(layout.replaceStructuralTiers||[]).filter(x=>String(x)!=='FLOOR');
  }
  layout.seatMapFingerprint=analysis.hash;
  layout.latestSeatLayoutSourceUrl=analysis.sourceUrl||layout.latestSeatLayoutSourceUrl||layout.sourceUrl;
  ...
  layout.autoMapImageSize={width:Number(analysis.width||0),height:Number(analysis.height||0)};
  layout.autoMapAnalysisVersion=analysis.analysisVersion||'0.41';
  layout.autoMapAnalyzedAt=new Date().toISOString();
  layout.sourceName=layout.autoGenerated?'官方座位圖自動解析＋完整場館基底':layout.sourceName;
  return true;
}
