...
function sourceFamily(ref={}){const s=`${ref.name||''} ${ref.url||''}`.toLowerCase();if(/tixcraft|kktix|ticketplus|kham|ibon|famiticket|udnfunlife|ticket\.mna|ticket\.com\.tw|tixfun|opentix|fansi|indievox|tickets\.books/.test(s))return'ticket';if(/場館|arena|ticc|tmc|zepp|music center|calendar|schedule/.test(s))return'venue';if(/livenation|live nation|主辦/.test(s))return'promoter';if(/weverse|ygfamily|jype|smtown|artist|藝人官方/.test(s))return'artist';return'other';}
...
export function auditCoverage({events=[],rawDiscovered=[],sourceHealth=[]}={}){
  ...
  const venueCandidates=rawDiscovered.filter(e=>/場館|calendar|schedule|TICC|TMC|Zepp/i.test(`${e.sourceName||''}`));
  const independentCandidates=rawDiscovered.filter(e=>!/場館|calendar|schedule|TICC|TMC|Zepp/i.test(`${e.sourceName||''}`));
