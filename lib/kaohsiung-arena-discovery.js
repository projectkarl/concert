...
import { koreanEventMarkers } from '../data/discovery.js';
...
function looksKorean(title=''){const h=title.toLowerCase();return /[가-힣]/.test(title)||koreanEventMarkers.some(m=>h.includes(m.toLowerCase()));}
...
export function parseKaohsiungArenaCalendar(html=''){
  ...
  for(const line of text.split(/\n+/).map(x=>x.trim()).filter(Boolean)){
    ...
    if(!m)continue;
    const title=m[1].trim(); if(!looksKorean(title))continue;
    ...
    out.push({id:`kao-${slug(title)}-${m[2]}${m[3]}${m[4]}`,artist,shortArtist:artist.replace(/[^A-Za-z0-9]/g,'').slice(0,3).toUpperCase()||'KR',title,type:'CONCERT',region:'TW',start,end,timeConfirmed:false,venue:'高雄巨蛋 Kaohsiung Arena',city:'Kaohsiung',statusLabel:'演唱會',ticketStatus:'CHECK OFFICIAL',generalSale:null,ticketing:'依主辦／官方售票平台公告',price:'依官方售票頁公告',sourceName:'高雄巨蛋官方活動行事曆',sourceUrl:CALENDAR_URL,sharedSourceUrl:true,verified:true,checkedAt:new Date().toISOString(),tags:['KOREA','KAOHSIUNG','OFFICIAL CALENDAR','AUTO'],summary:'高雄巨蛋官方活動行事曆已確認本場次；售票、舞台與入場資訊請回主辦及官方售票平台核對。',ticketTimeline:[],notes:['高雄巨蛋行事曆用於確認場次日期；完整票務規則以藝人、主辦與官方售票平台公告為準。'],venueModelId:'kaohsiung-arena',venueLayoutId:'kaohsiung-base'});
  }
  return out;
}
