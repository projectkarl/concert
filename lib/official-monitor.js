...
const OFFICIAL_TICKET_HOSTS = [
  ...
  'tickets.udnfunlife.com','ticket.com.tw','ticket.mna.com.tw','famiticket.com.tw','kham.com.tw','weverse.io'
];
...
export function extractOfficialSeatLayoutUrl(html='', baseUrl='') {
  ...
  const score=(url,context='')=>{
    ...
    if (/static\.tixcraft\.com\/images\/activity\/field\//.test(url.toLowerCase())) n+=8;
    if (/map|seat|field|plan|座位|票區|視線/.test(decodeURIComponent(url).toLowerCase())) n+=5;
    if (/\.(?:png|jpe?g|webp|pdf)(?:\?|$)/i.test(url)) n+=2;
    if (/static\.tixcraft\.com|tixcraft\.com|kktix\.(?:com|cc)|assets\.kktix\.io|ticketplus\.com\.tw/.test(url)) n+=2;
    return n;
  };
...
export async function monitorOfficialSource(seed, { fetchImpl = fetch, timeoutMs = 4500 } = {}) {
  ...
  const settled=await Promise.allSettled(supportedUrls.map(async url=>{
    const kind=sourceKind(url); const response=await fetchImpl(url,{headers:{"Accept":"text/html,application/xhtml+xml","Accept-Language":"zh-TW,zh;q=0.9,en;q=0.8","User-Agent":"NEUL/0.41 (+public-event-monitor)"},redirect:"follow",signal:AbortSignal.timeout(timeoutMs)});
