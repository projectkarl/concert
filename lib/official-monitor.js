...
const OFFICIAL_TICKET_HOSTS = [
  'livenation.com.tw','tixcraft.com','static.tixcraft.com','kktix.com','kktix.cc','assets.kktix.io','ibon.com.tw','ticketplus.com.tw',
  'tickets.udnfunlife.com','ticket.com.tw','ticket.mna.com.tw','famiticket.com.tw','kham.com.tw','opentix.life','tixfun.com','go.fansi.me','tickets.books.com.tw','weverse.io'
];
...
export function extractOfficialSeatLayoutUrl(html='', baseUrl='') {
  ...
  const score=(url,context='')=>{
    ...
    let n=0;
    if (/seat\s*map|seating|seatmap|field|venue\s*map|座位|票區|座席|場地圖|配置圖|平面圖|席次圖|座位配置/.test(hay)) n+=7;
    if (/static\.tixcraft\.com\/images\/activity\/field\//.test(url.toLowerCase())) n+=12;
    if (/map|seat|field|plan|layout|zone|座位|票區|視線|場地/.test(hay)) n+=5;
    if (/\.(?:png|jpe?g|webp|avif)(?:\?|$)/i.test(url)) n+=4;
    if (/poster|banner|logo|artist|kv|keyvisual|header|share|ogimage/i.test(decoded.toLowerCase())) n-=7;
    if (/static\.tixcraft\.com|tixcraft\.com|kktix\.(?:com|cc)|assets\.kktix\.io|ticketplus\.com\.tw|kham\.com\.tw|ticket\.ibon\.com\.tw|famiticket\.com\.tw|udnfunlife\.com|ticket\.mna\.com\.tw|ticket\.com\.tw|opentix\.life|tixfun\.com|go\.fansi\.me|tickets\.books\.com\.tw/.test(url)) n+=3;
    return n;
  };
  const push=(raw,context='')=>{const url=officialishUrl(String(raw||'').replace(/&amp;/g,'&'),baseUrl);if(url)candidates.push({url,score:score(url,context)});};
  const attrRe=/(?:href|src|data-src|data-original|data-lazy|data-image|data-url)\s*=\s*["']([^"']+)["']/gi;
  for (const m of source.matchAll(attrRe)) push(m[1],source.slice(Math.max(0,(m.index||0)-260),(m.index||0)+900));
  ...
  for (const m of source.matchAll(/url\((?:["']?)([^)"']+)(?:["']?)\)/gi)) push(m[1],source.slice(Math.max(0,(m.index||0)-180),(m.index||0)+400));
  for (const raw of source.match(/https?:\/\/[^\s"'<>\\]+/gi)||[]) {const i=source.indexOf(raw);push(raw,i>=0?source.slice(Math.max(0,i-220),i+raw.length+220):'');}
  const unique=new Map();for(const c of candidates){const old=unique.get(c.url);if(!old||c.score>old.score)unique.set(c.url,c);}const ranked=[...unique.values()].sort((a,b)=>b.score-a.score);
...
function parseTicketPage(html, seed, url='') {
  ...
  return {patch,check:{status:(dateOk||titleOk)&&venueOk?'live':'review',dateOk,venueOk,titleOk,message:(dateOk||titleOk)&&venueOk?'官方售票頁已連線':'官方售票頁內容可能已調整'}};
}

export async function monitorOfficialSource(seed, { fetchImpl = fetch, timeoutMs = 4500 } = {}) {
  const urls=[seed.sourceUrl,seed.secondarySourceUrl].filter(Boolean).filter((x,i,a)=>a.indexOf(x)===i);
