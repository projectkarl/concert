...
const UA = "Mozilla/5.0 (compatible; NEUL/0.37; +https://vercel.app)";
...
const PLATFORM_INDEXES = [
  ...
  {name:"KKTIX",url:"https://kktix.com/events",detail:/https?:\/\/[A-Za-z0-9.-]*kktix\.cc\/events\/[A-Za-z0-9_-]+/gi,maxDetails:32},
...
export async function discoverTaiwanTicketPlatforms(){
  ...
  const indexes=await Promise.allSettled(PLATFORM_INDEXES.map(async platform=>({platform,html:await getText(platform.url)})));
  checkedUrls+=PLATFORM_INDEXES.length;
  for(const result of indexes){
    if(result.status!=="fulfilled"){
      indexErrors.push(result.reason?.message||"ticket index unavailable");
      continue;
    }
    const {platform,html}=result.value;
    const urls=linksFrom(html,platform.url,platform).slice(0,platform.maxDetails||24);
  ...
  return{events,checkedUrls,indexErrors,pageErrors,source:"tixCraft + KKTIX + Ticket Plus + 寬宏 + FamiTicket + udn + ibon + MNA + 年代"};
}
