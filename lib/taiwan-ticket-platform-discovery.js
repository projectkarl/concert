...
const PLATFORM_INDEXES = [
  ...
  {name:"KKTIX",url:"https://kktix.com/events",indexes:[...Array.from({length:20},(_,i)=>`https://kktix.com/events?page=${i+1}`),"https://chuanyeah.kktix.cc/events","https://wve.kktix.cc/","https://kklivetw.kktix.cc/events","https://atc-twn.kktix.cc/"],detail:/https?:\/\/[A-Za-z0-9.-]*kktix\.cc\/events\/[A-Za-z0-9_-]+/gi,maxDetails:96},
...
export async function discoverTaiwanTicketPlatforms(){
  ...
  return{events,checkedUrls,indexErrors,pageErrors,source:"tixCraft + KKTIX + Ticket Plus + 寬宏 + FamiTicket + udn + ibon + MNA + 年代 + TixFun + OPENTIX + FANSI GO + iNDIEVOX + 博客來",sourceHealth:PLATFORM_INDEXES.map(p=>({name:p.name,indexCount:(p.indexes||[p.url]).length,detailCap:p.maxDetails||24,discovered:[...(urlsByPlatform.get(p.name)||[])].length,indexDerived:p.name==='iNDIEVOX'?indexEvents.length:0}))};
}
