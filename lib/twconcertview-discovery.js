...
function extractCalendarPageLinks(html='',base=CALENDAR_EN_URL){
  const out=new Set();
  const source=decodeEntities(String(html||''));
  for(const m of source.matchAll(/href=["']([^"']+)["']/gi)){
    try{
      const u=new URL(m[1],base);
      if(!/(?:^|\.)twconcertview\.com$/i.test(u.hostname)) continue;
      if(!/\/calendar(?:\/|$)/i.test(u.pathname)) continue;
      if(u.href===CALENDAR_URL||u.href===CALENDAR_EN_URL) continue;
      out.add(u.href);
    }catch{}
  }
...
function futureReferenceEvents(events=[],now=Date.now()){
  const today=new Date(now-12*3600000);
  return (events||[]).filter(e=>{const ts=new Date(e.start||0).getTime();return Number.isFinite(ts)&&ts>=today.getTime();});
}
...
export async function discoverTwConcertViewCalendar(){
  const urls=buildTwConcertViewCalendarUrls(new Date(),1,18);
  const pageErrors=[];
  try{
    const settled=await inBatches(urls,async url=>getCalendar(url),6);
    const parsed=[]; const discoveredLinks=new Set(); let successCount=0;
    for(let i=0;i<settled.length;i++){
      const result=settled[i], requested=urls[i];
      if(result.status!=='fulfilled'){
        pageErrors.push(result.reason?.message||`twconcertview unavailable ${requested}`);
        continue;
      }
      successCount++;
      for(const link of extractCalendarPageLinks(result.value.html,result.value.url||requested)) discoveredLinks.add(link);
    ...
    // Follow the calendar URLs emitted by the live DOM instead of assuming a single query-string format.
    // This survives calendar-router changes and is bounded to avoid excessive serverless work.
    const extraUrls=[...discoveredLinks].filter(x=>!urls.includes(x)).slice(0,24);
    if(extraUrls.length){
      const extraSettled=await inBatches(extraUrls,async url=>getCalendar(url),6);
      for(let i=0;i<extraSettled.length;i++){
        const result=extraSettled[i], requested=extraUrls[i];
    ...
    const mergedRaw=mergeCalendarPages(parsed);
    const currentEvents=futureReferenceEvents(mergedRaw.events);
    const merged={...mergedRaw,events:currentEvents,parsedCount:currentEvents.length,coverageRatio:mergedRaw.referenceCount>0?Math.min(1,currentEvents.length/mergedRaw.referenceCount):null};
    const conservativeComplete=merged.referenceCount>0 && merged.parsedCount>=Math.ceil(merged.referenceCount*0.9);
    return {
      ...merged,
      checkedUrls:urls.length+extraUrls.length,
      successfulPages:successCount,
      monthsScanned:Math.max(0,urls.length-1),
      dynamicCalendarPages:extraUrls.length,
      completeAgainstReference:conservativeComplete,
      source:'twconcertview 公開行事曆（逐月 coverage cross-check）',
    ...
    return {events:[],referenceCount:0,parsedCount:0,coverageRatio:null,completeAgainstReference:false,checkedUrls:urls.length,successfulPages:0,monthsScanned:Math.max(0,urls.length-1),source:'twconcertview 公開行事曆（逐月 coverage cross-check）',pageErrors:[error?.message||'twconcertview unavailable'],indexErrors:[],sourceHealth:[{name:'twconcertview coverage cross-check',discovered:0,referenceCount:0,coverageRatio:null,completeAgainstReference:false,checkedUrls:urls.length,successfulPages:0,errors:1}]};
  }
}
