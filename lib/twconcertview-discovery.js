...
const UA = 'Mozilla/5.0 (compatible; NEUL/0.40.16; +coverage-reference-queue)';
...
export async function discoverTwConcertViewCalendar(){
  const now=new Date(),pageErrors=[],parsed=[];let checkedUrls=0,zhMonthPages=0,enFallbackPages=0;
  try{
    ...
    try{const root=await getCalendar(CALENDAR_URL);checkedUrls++;parsed.push(parseTwConcertViewCalendar(root.html,{sourceUrl:root.url||CALENDAR_URL}));}
    ...
    let merged=mergeCalendarPages(parsed),showingParsedCount=merged.referenceOccurrenceCount||0;
    ...
    if(!rootComplete){
      ...
      for(let i=0;i<zhSettled.length;i++){
        ...
        zhSuccess++;try{parsed.push(parseTwConcertViewCalendar(result.value.html,{sourceUrl:result.value.url||requested}));}catch(error){pageErrors.push(`TWCV_PARSE ${requested} ${error?.message||error}`);}
      }
      checkedUrls+=zhMonthUrls.length;zhMonthPages=zhMonthUrls.length;merged=mergeCalendarPages(parsed);showingParsedCount=merged.referenceOccurrenceCount||0;
      ...
      if(zhSuccess===0){
        ...
        checkedUrls+=enMonthUrls.length;enFallbackPages=enMonthUrls.length;merged=mergeCalendarPages(parsed);showingParsedCount=merged.referenceOccurrenceCount||0;
      }
    }
    const completeAgainstReference=merged.referenceCount>0 && showingParsedCount>=Math.ceil(merged.referenceCount*0.9);
    return {...merged,checkedUrls,successfulPages:parsed.length,monthsScanned:zhMonthPages+enFallbackPages,rootFastPathComplete:rootComplete,completeAgainstReference,referenceUnit:'showings',showingParsedCount,uniqueEventCount:merged.parsedCount,uniqueEventRatio:merged.uniqueEventRatio,referenceQueueCount:merged.events.filter(x=>x.referenceOnly).length,zhMonthPages,enFallbackPages,source:'twconcertview 公開行事曆（補漏參考佇列；繁中主來源、英文僅備援）',pageErrors,indexErrors:[],sourceHealth:[{name:'twconcertview reference queue',discovered:merged.parsedCount,referenceCount:merged.referenceCount,referenceUnit:'showings',referenceQueueCount:merged.events.filter(x=>x.referenceOnly).length,uniqueEventCount:merged.parsedCount,showingParsedCount,uniqueEventRatio:merged.uniqueEventRatio,zhMonthPages,enFallbackPages,coverageRatio:merged.coverageRatio,completeAgainstReference,rootFastPathComplete:rootComplete,checkedUrls,successfulPages:parsed.length,errors:pageErrors.length}]};
  }catch(error){
