...
if(pkg.version!=='0.40.6') fail('package must be 0.40.6',pkg.version);
if(MAINSTREAM_3D_VENUE_IDS.length!==10) fail('mainstream 3D venue list must be exactly 10',MAINSTREAM_3D_VENUE_IDS.length);
if(new Set(MAINSTREAM_3D_VENUE_IDS).size!==10) fail('mainstream 3D venue ids contain duplicates');
...
for(const id of ['taoyuan-arena','ntu-sports-center','tianmu-gymnasium']){
...
if(!/MAINSTREAM_3D_VENUES\.has\(v\.id\)/.test(app)) fail('venue dropdown is not restricted to mainstream 10');
...
for(const host of ['farglorydome.com.tw','ntsu.edu.tw','ticc.com.tw','tainex.com.tw','zepp.co.jp','kcg.gov.tw','kph.tw']) if(!resolver.includes(host)) fail(`official seat-map allowlist missing ${host}`);
...
const excludedSeed=seedEvents.filter(e=>['taoyuan-arena','ntu-sports-center','tianmu-gymnasium'].includes(e.venueModelId));
if(!excludedSeed.length) fail('exclusion fixture events missing from seed data');
...
console.log(`NEUL v0.40.6 checks passed · 10 mainstream 3D venues · News bottom nav/section · uniform 範例 labels · reference official-map preview · lower-priority 3D disabled`);
