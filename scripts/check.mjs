...
if (!/約每 6 小時/.test(app) || !/每日排程同步/.test(app) || !/startAutomaticEventVerification/.test(app)) { console.error("update cadence label missing"); ok=false; }
...
if (!/\/i18n\.js/.test(sw) || !/neul-v0\.40\.(?:2-(?:full-coverage-auditor|complete(?:2)?-auto3d-calendar|ux1-calendar-seat3d|ux2-entertainment-news|ux3-news-venue-policy-officialmap)|3-venue-topology-guard|4-coverage-precision-mobile|5-kstar-reference-source-trace|6-mainstream10-news-bottom-official-map|7-mainstream12-news-bottom-official-map|8-aespa-official-seatmap|9-full-concert-coverage|10-3dprecision-nonregression|11-officialmap-cdn-fix|13-verified316-recentdata|14-cpu-saver)/.test(sw)) { console.error("PWA multilingual cache update missing"); ok=false; }
...
if (!/hydrateSeatMapGeometry/.test(app) || !/neul-seatmap-hash/.test(app) || !/hashChanged/.test(app) || /slice\(0,14\)/.test(app) || !/SEATMAP_BACKGROUND_MAX/.test(app) || !/interactive:true/.test(app)) { console.error("client seat-map lazy regeneration pipeline missing"); ok=false; }
...
if (!/neul-v0\.40\.(?:2-(?:full-coverage-auditor|complete(?:2)?-auto3d-calendar|ux1-calendar-seat3d|ux2-entertainment-news|ux3-news-venue-policy-officialmap)|3-venue-topology-guard|4-coverage-precision-mobile|5-kstar-reference-source-trace|6-mainstream10-news-bottom-official-map|7-mainstream12-news-bottom-official-map|8-aespa-official-seatmap|9-full-concert-coverage|10-3dprecision-nonregression|11-officialmap-cdn-fix|13-verified316-recentdata|14-cpu-saver)/.test(sw) || !/seat-map-intelligence\.js/.test(sw)) { console.error("v0.40 service-worker cache regression"); ok=false; }
